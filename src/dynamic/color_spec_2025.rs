...
impl ColorSpec for ColorSpec2025 {
        ...
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                if s.platform == Platform::Phone {
                    if s.is_dark { 4.0 }
                    else if Hct::is_yellow(s.neutral_palette.hue) { 99.0 }
                    else if s.variant == Variant::Vibrant { 97.0 }
                    else { 98.0 }
                } else { 0.0 }
            })),
            None, None, None, None,
    ...
    fn background(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true, None, None,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).surface().get_tone(s))),
            None, None, None, None,
        );
    ...
    fn on_background(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            false, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).on_surface().background.as_ref().and_then(|f| f(s))?))),
            Some(Arc::new(|s| {
                if s.platform == Platform::Watch { 100.0 }
                else { ColorSpecs::get(s.spec_version).on_surface().get_tone(s) }
            })),
            None,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).on_surface().contrast_curve.as_ref().and_then(|f| f(s)))),
            None, None,
        );
    ...
    fn surface_dim(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            None,
            Some(Arc::new(|s| if s.is_dark { 4.0 }
            else if Hct::is_yellow(s.neutral_palette.hue) { 90.0 }
            else if s.variant == Variant::Vibrant { 85.0 }
            else { 87.0 }
            )),
            None, None, None, None,
        );
    ...
    fn surface_bright(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            None,
            Some(Arc::new(|s| if s.is_dark { 18.0 }
            else if Hct::is_yellow(s.neutral_palette.hue) { 99.0 }
            else if s.variant == Variant::Vibrant { 97.0 }
            else { 98.0 }
            )),
    ...
    fn surface_container_low(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Arc::new(|s| s.neutral_palette.clone()),
            true,
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                match s.variant {
                    Variant::Neutral => 1.3,
            ...
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                if s.is_dark { 6.0 }
                else if Hct::is_yellow(s.neutral_palette.hue) { 98.0 }
                else if s.variant == Variant::Vibrant { 95.0 }
                else { 96.0 }
            } else { 15.0 })),
            None, None, None, None,
        );
    ...
    fn surface_container(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                if s.is_dark { 9.0 }
                else if Hct::is_yellow(s.neutral_palette.hue) { 96.0 }
                else if s.variant == Variant::Vibrant { 92.0 }
                else { 94.0 }
            } else { 20.0 })),
            None, None, None, None,
        );
    ...
    fn surface_container_high(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                if s.is_dark { 12.0 }
                else if Hct::is_yellow(s.neutral_palette.hue) { 94.0 }
                else if s.variant == Variant::Vibrant { 90.0 }
                else { 92.0 }
            } else { 25.0 })),
            None, None, None, None,
        );
    ...
    fn surface_container_highest(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            None,
            Some(Arc::new(|s| if s.is_dark { 15.0 }
            else if Hct::is_yellow(s.neutral_palette.hue) { 92.0 }
            else if s.variant == Variant::Vibrant { 88.0 }
            else { 90.0 }
            )),
            None, None, None, None,
        );
    ...
    fn on_surface(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                if s.variant == Variant::Vibrant { Self::t_max_c(&s.neutral_palette, 0.0, 100.0, 1.1) }
                else { ColorSpecs::get(s.spec_version).highest_surface(s).get_tone(s) }
            })),
            None,
    ...
    fn surface_variant(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Arc::new(|s| ColorSpecs::get(s.spec_version).surface_container_highest().palette.clone()(s)),
            true,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).surface_container_highest().chroma_multiplier.as_ref().map_or(1.0, |f| f(s)))),
            None,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).surface_container_highest().get_tone(s))),
            None, None, None, None,
        );
    ...
    fn surface_tint(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).primary().chroma_multiplier.as_ref().map_or(1.0, |f| f(s)))),
            None,
            Some(Arc::new(|s| ColorSpecs::get(s.spec_version).primary().get_tone(s))),
            None, None, None, None,
        );
    ...
    fn primary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).highest_surface(s)))),
            Some(Arc::new(|s| match s.variant {
                Variant::Neutral => if s.platform == Platform::Phone { if s.is_dark { 80.0 } else { 40.0 } } else { 90.0 },
                Variant::TonalSpot => if s.platform == Platform::Phone { if s.is_dark { 80.0 } else { Self::t_max_c(&s.primary_palette, 0.0, 100.0, 1.0) } } else { Self::t_max_c(&s.primary_palette, 0.0, 90.0, 1.0) },
                Variant::Expressive => if s.platform == Platform::Phone { let hue = s.primary_palette.hue; let upper = if Hct::is_yellow(hue) { 25.0 } else if Hct::is_cyan(hue) { 88.0 } else { 98.0 }; Self::t_max_c(&s.primary_palette, 0.0, upper, 1.0) } else { Self::t_max_c(&s.primary_palette, 0.0, 100.0, 1.0) },
                _ => if s.platform == Platform::Phone { let upper = if Hct::is_cyan(s.primary_palette.hue) { 88.0 } else { 98.0 }; Self::t_max_c(&s.primary_palette, 0.0, upper, 1.0) } else { Self::t_max_c(&s.primary_palette, 0.0, 100.0, 1.0) },
            })),
            None,
            ...
            Some(Arc::new(|s| {
                ...
                if s.platform == Platform::Phone {
                    Some(ToneDeltaPair::new(spec.primary_container(), spec.primary(), 5.0, TonePolarity::RelativeLighter, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn primary_dim(&self) -> Option<Arc<DynamicColor>> {
        Some(Arc::new(DynamicColor::new(
            ...
            Arc::new(|s| s.primary_palette.clone()),
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).surface_container_high()))),
            Some(Arc::new(|s| match s.variant {
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.primary_dim()?, spec.primary(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
            })),
            None,
        )))
    }
    ...
    fn on_primary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Phone { Some(spec.primary()) } else { spec.primary_dim() }
            })),
            None, None,
    ...
    fn primary_container(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Arc::new(|s| s.primary_palette.clone()),
            true, None,
            Some(Arc::new(|s| if s.platform == Platform::Phone { Some(ColorSpecs::get(s.spec_version).highest_surface(s)) } else { None })),
            Some(Arc::new(|s| {
                if s.platform == Platform::Watch { 30.0 }
                else { match s.variant {
                    Variant::Neutral => if s.is_dark { 30.0 } else { 90.0 },
                    Variant::TonalSpot => if s.is_dark { Self::t_min_c(&s.primary_palette, 35.0, 93.0) } else { Self::t_max_c(&s.primary_palette, 0.0, 90.0, 1.0) },
                    Variant::Expressive => if s.is_dark { Self::t_max_c(&s.primary_palette, 30.0, 93.0, 1.0) } else { let upper = if Hct::is_cyan(s.primary_palette.hue) { 88.0 } else { 90.0 }; Self::t_max_c(&s.primary_palette, 78.0, upper, 1.0) },
                    _ => if s.is_dark { Self::t_min_c(&s.primary_palette, 66.0, 93.0) } else { let upper = if Hct::is_cyan(s.primary_palette.hue) { 88.0 } else { 93.0 }; Self::t_max_c(&s.primary_palette, 66.0, upper, 1.0) },
                }}
            })),
            None,
            ...
            Some(Arc::new(|s| {
                ...
                if s.platform == Platform::Watch {
                    Some(ToneDeltaPair::new(spec.primary_container(), spec.primary_dim()?, 10.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn inverse_primary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            false, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).inverse_surface()))),
            Some(Arc::new(|s| Self::t_max_c(&s.primary_palette, 0.0, 100.0, 1.0))),
            None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 6.0 } else { 7.0 })))),
            None, None,
        );
    ...
    fn secondary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).highest_surface(s)))),
            Some(Arc::new(|s| if s.platform == Platform::Watch { if s.variant == Variant::Neutral { 90.0 } else { Self::t_max_c(&s.secondary_palette, 0.0, 90.0, 1.0) } }
            else { match s.variant {
                Variant::Neutral => if s.is_dark { Self::t_min_c(&s.secondary_palette, 0.0, 98.0) } else { Self::t_max_c(&s.secondary_palette, 0.0, 100.0, 1.0) },
                Variant::Vibrant => Self::t_max_c(&s.secondary_palette, 0.0, if s.is_dark { 90.0 } else { 98.0 }, 1.0),
                _ => if s.is_dark { 80.0 } else { Self::t_max_c(&s.secondary_palette, 0.0, 100.0, 1.0) },
            }}
            )),
            None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 4.5 } else { 7.0 })))),
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.secondary_container(), spec.secondary(), 5.0, TonePolarity::RelativeLighter, true, DeltaConstraint::Farther))
            } else { None })),
            None,
        );
    ...
    fn secondary_dim(&self) -> Option<Arc<DynamicColor>> {
        Some(Arc::new(DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).surface_container_high()))),
            Some(Arc::new(|s| if s.variant == Variant::Neutral { 85.0 } else { Self::t_max_c(&s.secondary_palette, 0.0, 90.0, 1.0) })),
            None,
            Some(Arc::new(|_| Some(Self::get_contrast_curve(4.5)))),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.secondary_dim()?, spec.secondary(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
            })),
            None,
        )))
    }
    ...
    fn on_secondary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Phone { Some(spec.secondary()) } else { spec.secondary_dim() }
            })),
            None, None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 6.0 } else { 7.0 })))),
            None, None,
    ...
    fn secondary_container(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| if s.platform == Platform::Phone { Some(ColorSpecs::get(s.spec_version).highest_surface(s)) } else { None })),
            Some(Arc::new(|s| if s.platform == Platform::Watch { 30.0 } else {
                match s.variant {
                    Variant::Vibrant => if s.is_dark { Self::t_min_c(&s.secondary_palette, 30.0, 40.0) } else { Self::t_max_c(&s.secondary_palette, 84.0, 90.0, 1.0) },
                    Variant::Expressive => if s.is_dark { 15.0 } else { Self::t_max_c(&s.secondary_palette, 90.0, 95.0, 1.0) },
                    _ => if s.is_dark { 25.0 } else { 90.0 },
                }
            })),
            None,
            Some(Arc::new(|s| if s.platform == Platform::Phone && s.contrast_level > 0.0 { Some(Self::get_contrast_curve(1.5)) } else { None })),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Watch {
                    Some(ToneDeltaPair::new(spec.secondary_container(), spec.secondary_dim()?, 10.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn tertiary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).highest_surface(s)))),
            Some(Arc::new(|s| if s.platform == Platform::Watch { if s.variant == Variant::TonalSpot { Self::t_max_c(&s.tertiary_palette, 0.0, 90.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 0.0, 100.0, 1.0) } }
            else { match s.variant {
                Variant::Expressive | Variant::Vibrant => { let upper = if Hct::is_cyan(s.tertiary_palette.hue) { 88.0 } else if s.is_dark { 98.0 } else { 100.0 }; Self::t_max_c(&s.tertiary_palette, 0.0, upper, 1.0) },
                _ => if s.is_dark { Self::t_max_c(&s.tertiary_palette, 0.0, 98.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 0.0, 100.0, 1.0) },
            }}
            )),
            None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 4.5 } else { 7.0 })))),
            Some(Arc::new(|s| if s.platform == Platform::Phone {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.tertiary_container(), spec.tertiary(), 5.0, TonePolarity::RelativeLighter, true, DeltaConstraint::Farther))
            } else { None })),
            None,
        );
    ...
    fn tertiary_dim(&self) -> Option<Arc<DynamicColor>> {
        Some(Arc::new(DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).surface_container_high()))),
            Some(Arc::new(|s| if s.variant == Variant::TonalSpot { Self::t_max_c(&s.tertiary_palette, 0.0, 90.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 0.0, 100.0, 1.0) } )),
            None,
            Some(Arc::new(|_| Some(Self::get_contrast_curve(4.5)))),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.tertiary_dim()?, spec.tertiary(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
            })),
            None,
        )))
    }
    ...
    fn on_tertiary(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Phone { Some(spec.tertiary()) } else { spec.tertiary_dim() }
            })),
            None, None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 6.0 } else { 7.0 })))),
            None, None,
    ...
    fn tertiary_container(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| if s.platform == Platform::Watch { if s.variant == Variant::TonalSpot { Self::t_max_c(&s.tertiary_palette, 0.0, 90.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 0.0, 100.0, 1.0) } }
                ...
                Variant::Neutral => if s.is_dark { Self::t_max_c(&s.tertiary_palette, 0.0, 93.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 0.0, 96.0, 1.0) },
                Variant::TonalSpot => Self::t_max_c(&s.tertiary_palette, 0.0, if s.is_dark { 93.0 } else { 100.0 }, 1.0),
                Variant::Expressive => { let upper = if Hct::is_cyan(s.tertiary_palette.hue) { 88.0 } else if s.is_dark { 93.0 } else { 100.0 }; Self::t_max_c(&s.tertiary_palette, 75.0, upper, 1.0) },
                _ => if s.is_dark { Self::t_max_c(&s.tertiary_palette, 0.0, 93.0, 1.0) } else { Self::t_max_c(&s.tertiary_palette, 72.0, 100.0, 1.0) },
            }}
            )),
            None,
            Some(Arc::new(|s| if s.platform == Platform::Phone && s.contrast_level > 0.0 { Some(Self::get_contrast_curve(1.5)) } else { None })),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Watch {
                    Some(ToneDeltaPair::new(spec.tertiary_container(), spec.tertiary_dim()?, 10.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn error(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            true, None,
            Some(Arc::new(|s| Some(ColorSpecs::get(s.spec_version).highest_surface(s)))),
            Some(Arc::new(|s| if s.platform == Platform::Phone { if s.is_dark { Self::t_min_c(&s.error_palette, 0.0, 98.0) } else { Self::t_max_c(&s.error_palette, 0.0, 100.0, 1.0) } }
            else { Self::t_min_c(&s.error_palette, 0.0, 100.0) }
            )),
            None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 4.5 } else { 7.0 })))),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Phone {
                    Some(ToneDeltaPair::new(spec.error_container(), spec.error(), 5.0, TonePolarity::RelativeLighter, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn error_dim(&self) -> Option<Arc<DynamicColor>> {
        Some(Arc::new(DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.error_dim()?, spec.error(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
            })),
            None,
        )))
    }
    ...
    fn on_error(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Phone { Some(spec.error()) } else { spec.error_dim() }
            })),
            None, None,
            Some(Arc::new(|s| Some(Self::get_contrast_curve(if s.platform == Platform::Phone { 6.0 } else { 7.0 })))),
            None, None,
    ...
    fn error_container(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| if s.platform == Platform::Phone { Some(ColorSpecs::get(s.spec_version).highest_surface(s)) } else { None })),
            Some(Arc::new(|s| if s.platform == Platform::Watch { 30.0 } else {
                if s.is_dark { Self::t_min_c(&s.error_palette, 30.0, 93.0) } else { Self::t_max_c(&s.error_palette, 0.0, 90.0, 1.0) }
            })),
            None,
            Some(Arc::new(|s| if s.platform == Platform::Phone && s.contrast_level > 0.0 { Some(Self::get_contrast_curve(1.5)) } else { None })),
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                if s.platform == Platform::Watch {
                    Some(ToneDeltaPair::new(spec.error_container(), spec.error_dim()?, 10.0, TonePolarity::Darker, true, DeltaConstraint::Farther))
                } else { None }
            })),
            None,
        );
    ...
    fn primary_fixed_dim(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.primary_fixed_dim(), spec.primary_fixed(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Exact))
            })),
            None,
        );
    ...
    fn secondary_fixed_dim(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.secondary_fixed_dim(), spec.secondary_fixed(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Exact))
            })),
            None,
        );
    ...
    fn tertiary_fixed_dim(&self) -> Arc<DynamicColor> {
        let color2025 = DynamicColor::new(
            ...
            Some(Arc::new(|s| {
                let spec = ColorSpecs::get(s.spec_version);
                Some(ToneDeltaPair::new(spec.tertiary_fixed_dim(), spec.tertiary_fixed(), 5.0, TonePolarity::Darker, true, DeltaConstraint::Exact))
            })),
            None,
        );
        ...
        if let Some(tdp) = tone_delta_pair {
            ...
            match tdp.constraint {
                ...
                DeltaConstraint::Nearer => {
                    if relative_delta > 0.0 { self_tone = self_tone.clamp(reference_tone, (reference_tone + relative_delta).max(reference_tone)).clamp(0.0, 100.0); }
                    else { self_tone = self_tone.clamp((reference_tone + relative_delta).min(reference_tone), reference_tone).clamp(0.0, 100.0); }
                }
                DeltaConstraint::Farther => {
                    if relative_delta > 0.0 { self_tone = self_tone.clamp((reference_tone + relative_delta).min(100.0), 100.0); }
                    else { self_tone = self_tone.clamp(0.0, (reference_tone + relative_delta).max(0.0)); }
                }
            }
            ...
            if color.is_background && !color.name.ends_with("_fixed_dim") {
                self_tone = if self_tone >= 57.0 { self_tone.clamp(65.0, 100.0) } else { self_tone.clamp(0.0, 49.0) };
            }
            self_tone
        } else {
            ...
            if let Some(bg2_fn) = color.second_background.as_ref() {
                if let (Some(bg1), Some(bg2), Some(cc_fn)) = (color.background.as_ref().and_then(|f| f(scheme)), bg2_fn(scheme), color.contrast_curve.as_ref().and_then(|f| f(scheme))) {
                    let (t1, t2) = (bg1.get_tone(scheme), bg2.get_tone(scheme));
                    let desired = cc_fn.get(scheme.contrast_level);
                    if Contrast::ratio_of_tones(t1.max(t2), answer) < desired || Contrast::ratio_of_tones(t1.min(t2), answer) < desired {
                        let light = Contrast::lighter(t1.max(t2), desired);
                        let dark = Contrast::darker(t1.min(t2), desired);
                        if DynamicColor::tone_prefers_light_foreground(t1) || DynamicColor::tone_prefers_light_foreground(t2) {
    ...
    fn get_primary_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        match variant {
            Variant::Neutral => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { if Hct::is_blue(source_color_hct.hue()) { 12.0 } else { 8.0 } } else { if Hct::is_blue(source_color_hct.hue()) { 16.0 } else { 12.0 } }),
            Variant::TonalSpot => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone && is_dark { 26.0 } else { 32.0 }),
            Variant::Expressive => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { if is_dark { 36.0 } else { 48.0 } } else { 40.0 }),
            Variant::Vibrant => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { 74.0 } else { 56.0 }),
    ...
    fn get_secondary_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        match variant {
            Variant::Neutral => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { if Hct::is_blue(source_color_hct.hue()) { 6.0 } else { 4.0 } } else { if Hct::is_blue(source_color_hct.hue()) { 10.0 } else { 6.0 } }),
            Variant::TonalSpot => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), 16.0),
            Variant::Expressive => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 105.0, 140.0, 204.0, 253.0, 278.0, 300.0, 333.0, 360.0], &[-160.0, 155.0, -100.0, 96.0, -96.0, -156.0, -165.0, -160.0]), if platform == Platform::Phone { if is_dark { 16.0 } else { 24.0 } } else { 24.0 }),
            Variant::Vibrant => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 38.0, 105.0, 140.0, 333.0, 360.0], &[-14.0, 10.0, -14.0, 10.0, -14.0]), if platform == Platform::Phone { 56.0 } else { 36.0 }),
    ...
    fn get_tertiary_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        match variant {
            Variant::Neutral => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 38.0, 105.0, 161.0, 204.0, 278.0, 333.0, 360.0], &[-32.0, 26.0, 10.0, -39.0, 24.0, -15.0, -32.0]), if platform == Platform::Phone { 20.0 } else { 36.0 }),
            Variant::TonalSpot => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 20.0, 71.0, 161.0, 333.0, 360.0], &[-40.0, 48.0, -32.0, 40.0, -32.0]), if platform == Platform::Phone { 28.0 } else { 32.0 }),
            Variant::Expressive => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 105.0, 140.0, 204.0, 253.0, 278.0, 300.0, 333.0, 360.0], &[-165.0, 160.0, -105.0, 101.0, -101.0, -160.0, -170.0, -165.0]), 48.0),
            Variant::Vibrant => TonalPalette::from_hue_and_chroma(DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 38.0, 71.0, 105.0, 140.0, 161.0, 253.0, 333.0, 360.0], &[-72.0, 35.0, 24.0, -24.0, 62.0, 50.0, 62.0, -72.0]), 56.0),
    ...
    fn get_neutral_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        match variant {
            Variant::Neutral => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { 1.4 } else { 6.0 }),
            Variant::TonalSpot => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), if platform == Platform::Phone { 5.0 } else { 10.0 }),
            Variant::Expressive => {
                let h = DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 71.0, 124.0, 253.0, 278.0, 300.0, 360.0], &[10.0, 0.0, 10.0, 0.0, 10.0, 0.0]);
                let c = if platform == Platform::Phone { if is_dark { if Hct::is_yellow(h) { 6.0 } else { 14.0 } } else { 18.0 } } else { 12.0 };
                TonalPalette::from_hue_and_chroma(h, c)
            },
            Variant::Vibrant => {
                let h = DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 38.0, 105.0, 140.0, 333.0, 360.0], &[-14.0, 10.0, -14.0, 10.0, -14.0]);
                let c = if platform == Platform::Phone { 28.0 } else { if Hct::is_blue(h) { 28.0 } else { 20.0 } };
                TonalPalette::from_hue_and_chroma(h, c)
            },
    ...
    fn get_neutral_variant_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        match variant {
            Variant::Neutral => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), (if platform == Platform::Phone { 1.4 } else { 6.0 }) * 2.2),
            Variant::TonalSpot => TonalPalette::from_hue_and_chroma(source_color_hct.hue(), (if platform == Platform::Phone { 5.0 } else { 10.0 }) * 1.7),
            Variant::Expressive => {
                let h = DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 71.0, 124.0, 253.0, 278.0, 300.0, 360.0], &[10.0, 0.0, 10.0, 0.0, 10.0, 0.0]);
                let c = if platform == Platform::Phone { if is_dark { if Hct::is_yellow(h) { 6.0 } else { 14.0 } } else { 18.0 } } else { 12.0 };
                TonalPalette::from_hue_and_chroma(h, c * if h >= 105.0 && h < 125.0 { 1.6 } else { 2.3 })
            },
            Variant::Vibrant => {
                let h = DynamicScheme::get_rotated_hue(source_color_hct, &[0.0, 38.0, 105.0, 140.0, 333.0, 360.0], &[-14.0, 10.0, -14.0, 10.0, -14.0]);
                let c = if platform == Platform::Phone { 28.0 } else { if Hct::is_blue(h) { 28.0 } else { 20.0 } };
    ...
    fn get_error_palette(&self, variant: Variant, source_color_hct: &Hct, is_dark: bool, platform: Platform, contrast_level: f64) -> TonalPalette {
        let error_hue = DynamicScheme::get_piecewise_value(source_color_hct, &[0.0, 3.0, 13.0, 23.0, 33.0, 43.0, 153.0, 273.0, 360.0], &[12.0, 22.0, 32.0, 12.0, 22.0, 32.0, 22.0, 12.0]);
        match variant {

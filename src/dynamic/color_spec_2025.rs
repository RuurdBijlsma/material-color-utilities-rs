            ...
            match constraint {
                ...
                DeltaConstraint::Nearer => {
                    if relative_delta > 0.0 {
                        let min = reference_tone;
                        let max = (reference_tone + relative_delta).max(min);
                        self_tone = self_tone.clamp(min, max).clamp(0.0, 100.0);
                    } else {
                        let max = reference_tone;
                        let min = (reference_tone + relative_delta).min(max);
                        self_tone = self_tone.clamp(min, max).clamp(0.0, 100.0);
                    }
                }
                DeltaConstraint::Farther => {
                    if relative_delta > 0.0 {
                        let min = (reference_tone + relative_delta).min(100.0);
                        self_tone = self_tone.clamp(min, 100.0);
                    } else {
                        let max = (reference_tone + relative_delta).max(0.0);
                        self_tone = self_tone.clamp(0.0, max);
                    }
                }
            }
